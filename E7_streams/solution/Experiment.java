...
public class Experiment {
    ...
    public static void checkAllPositives(String customerId) {
        ...
        //Internal iteration
        boolean allPositivesAlt = christmasBank.getAllAccounts().stream().
                filter(account -> account.getCustomerId().equals(customerId)).
                allMatch(account -> account.getBalance().signum() >= 0);
    ...
    public static void awardBonusInternal() {
        ...
        christmasBank.getAllAccounts().parallelStream().
                filter(account -> account.getBalance().signum() > 0).
                forEach(account -> account.deposit(account.getBalance().divide(bonusReduction)));
